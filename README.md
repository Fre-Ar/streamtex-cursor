...
StreamTeX is a Python project that provides a modular "block"-based framework to dynamically generate HTML elements with customizable styles, specifically designed to enhance Streamlit applications. 

## Contents
This github provides 2 main folders: ```project_aiai18h```, ```template_project``` and ```streamtex_package```.
The ```streamtex_package``` folder contains the source files for the StreamTeX library.
The ```project_aiai18h``` folder is an example project.
The ```template_project``` folder is an folder which you can copy as a template to start any new project.

## Set up 
Create a folder for your StreamTeX projects. Download the ```streamtex_package``` folder into that folder.
Then, in your chosen python environment, cd to the ```streamtex_package``` folder and run ```pip install -r requirements.txt```. This will install streamlit and all other dependencies.
...
The ```blocks``` folder should contain all block python files (see the Blocks section below).
The ```static``` folder contains the statically-served files, such as images. You may NOT rename the ```static``` or ```images``` folders.
The ```custom``` folder contains the project specific files which define both library configuration and user-created custom styles. You may rename these folders and files.
...
```
This imports streamlit and adds the parent folder to the python path. This is essential for the project to be able to access the```streamtex_package``` folder.

After this, you can import StreamTeX (```import streamtex_package.src.streamtex as sx```), your ```custom.config``` file and your block files, set up your streamlit page through ```st.set_page_config()``` and other streamlit code. 
The final line should be ```python sx.st_book(module_list)```, where ```module_list``` is a list of the block modules to include in your Streamlit web book in the order of appearance.

## Blocks

These python files define the modular parts of your streamlit web book. There are two types: 
- pure html blocks, which contain a ```html_block()``` function which returns a string filled with html code. The StreamTeX library provides st_* helper functions to generate this html code using (possibly custom) styles. For an example, look in ```template_project\blocks\show_off.py``` (this file also shows all st_* functions and the different ways they can be used). To start any new html block, it is recommended to just make a copy of ```template_project\blocks\base.py```. 
- panel blocks, which contain ```build_panel()``` function, returning nothing. It instead uses streamlit's function (like ```st.markdown``` and ```st.image```) to create a reusable block. StreamTeX's helper functions can still be used alongside ```st.markdown``` in this type of block. or an example, look in ```template_project\blocks\panel_block.py```.

## Styles

The StreamTeX st_* functions allow for an extensive variety in styling through its style parameters. These parameters accept an object of the ```Style``` class, a wrapper class for css styles which allows for easily combining styles. For lists, there's a ```ListStyle``` subclass of ```Style``` which defines the icons of the bullet points used for lists. Each style also has an id, which is used for themes. Additionally, certain st_* functions which serve as containers of multiple blocks (like st_list and st_grid) can use a ```StyleGrid``` class object, which represents a matrix of styles. 
The StreamTeX librabry already provides an extensive array of atomic styles in ```streamtex_package.src.streamtex.styles```.
...
### Custom Styles
In ```custom/styles.py```, you may define a new Custom class, which will hold all new styles you may want to create and not available through StreamTeX. At the end of the python file, define a new class Styles, a subclass of ```streamtex_package.src.streamtex.styles.StreamTeX_Styles```, with a new property referencing your new custom styles class. For reference, see ```project_aiai18h\custom\styles.py```
...
A table of contents may be generated by using ```st_write()```'s ```toc_lvl``` and ```label``` parameters. A ToC will then appear on the side bar. In the ```config.py``` file, the user may also define where (if at all) a larger ToC should appear on the web book.

## Configuration
In ```custom/config.py```, you may change the library's configuration, such as chosen style theme dictionary and table of contents settings (positioning in the page, titles, styles, etc). See ```template_project\custom\config.py``` for an example.
...
### Zoom
In the sidebar, a dropdown menu is available to rescale the page to different sizes, from 10% to 200% of base size.

# Streamlit Version
- Temporarily, this project only works with streamlit version 1.41, a future patch is WIP to allow for the latest version to be used.
...
- A GitHub repository with your StreamTeX project folder, and the ```streamtex_package``` folder.
...
- name: Deploy StreamTeX App on Google Cloud VM
  ...
  tasks:
    ...
    - name: Copy only `streamtex_package` folder from the repo
      command: cp -r "{{ project_dir }}/repo/streamtex_package" "{{ project_dir }}/"
